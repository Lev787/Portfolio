...
function LevsMario() {
  ...
  window.addEventListener("message", function(e) { 
      if(e.data === "startGame") { 
          window.gameon = true; 
          if(window.data) window.data.playerpower = 1;
          setMap(1,1); 
      } else if (e.data && e.data.action === "startGame") {
          window.gameon = true; 
          if(!window.data && typeof resetData === "function") resetData();
          if(window.data) {
              window.data.playerpower = e.data.power ? e.data.power : 1;
          }
          setMap(e.data.part, e.data.lvl);
      } else if (e.data && e.data.action === "tryAgain") {
          if (typeof tryAgainGame === "function") tryAgainGame();
      } else if (e.data && e.data.action === "updateVolume") {
          if (window.AudioPlayer) {
              window.AudioPlayer.setVolume(e.data.section, e.data.value);
          }
      }
  });
