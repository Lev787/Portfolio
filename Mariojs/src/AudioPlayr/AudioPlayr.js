...
function AudioPlayr(settings) {
  ...
  */
  var version = "1.0",
      ...
      // Volume amounts for section grouping
      volume_sounds = 1,
      volume_theme = 1,
      ...
      // Web Audio API context for zero-latency SFX playback
      audioCtx = (typeof window !== 'undefined' && (window.AudioContext || window.webkitAudioContext)) ? new (window.AudioContext || window.webkitAudioContext)() : null,
  ...
  */
  ...
  var play = this.play = function(name_raw) {
    ...
    // Make sure we apply section volume
    sound.volume_real = (sound.section === "Themes" ? volume_theme : volume_sounds);

    if (audioCtx && audioBuffers[name_raw] && !muted) {
        if (audioCtx.state === "suspended") audioCtx.resume();
        var source = audioCtx.createBufferSource();
        source.buffer = audioBuffers[name_raw];
        var gainNode = audioCtx.createGain();
        gainNode.gain.value = sound.volume_real !== undefined ? sound.volume_real : 1;
        source.connect(gainNode);
        gainNode.connect(audioCtx.destination);
        source.start(0);
    } else {
        soundStop(sound);
        sound.volume = (!muted ? sound.volume_real : 0);
        
        // This plays the HTML5 element.
  ...
  this.playLocal = function(name_raw, location) {
    ...
    // Multiply by our new master section volume
    volume_real *= (sound.section === "Themes" ? volume_theme : volume_sounds);
  ...
  */
  ...
  this.setVolume = function(section, volume) {
    if (section === "Sounds") {
      volume_sounds = volume;
    } else if (section === "Themes") {
      volume_theme = volume;
      if (theme) {
        theme.volume = (!muted ? volume_theme : 0);
      }
    }
    
    // Update currently playing sounds
    for (var i in sounds) {
      if (sounds[i].section === section) {
        sounds[i].volume_real = volume;
        if (!muted) {
  ...
  */
  ...
  function createAudio(name, section) {
    ...
    sound.section = section;
    sound.volume_real = (section === "Themes" ? volume_theme : volume_sounds);
    ...
    // Set auto preload for HTML5 fallback
    sound.preload = "auto";
    
    // Fast Web Audio API preload for Sounds
    if (audioCtx && section === "Sounds") {
        var prefetchSrc = directory + "/" + section + "/" + filetypes[0] + "/" + name + "." + filetypes[0];
        fetch(prefetchSrc)
            .then(function(res) { return res.arrayBuffer(); })
            .then(function(buf) { return audioCtx.decodeAudioData(buf); })
            .then(function(decoded) { audioBuffers[name] = decoded; })
            .catch(function(e) { console.log("Prefetch error for " + name, e); });
    }
