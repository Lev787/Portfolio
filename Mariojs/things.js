...
function movePlayer(me) {
  ...
  if(me.keys.run != 0 && !me.crouching) {
    var dir = me.keys.run,
        ...
        adder = dir * (.1225 * (sprinting + 1)); // Increased from .098 (1.25x)
  ...
  // Otherwise slow down a bit/*, with a little more if crouching*/
    ...
    me.xvel *= .975; // Increased friction from .98
    decel = .044;    // Increased decel from .035
  }
...
function gameOver() {
  ...
  if (window.parent && window.parent !== window) {
      window.parent.postMessage({action: "gameOver_start"}, "*");
  }
  ...
  if (window.parent && window.parent !== window) {
      setTimeout(function() {
          body.innerHTML = ""; // Clear text to show solid black background
          window.parent.postMessage({action: "gameOver_ui"}, "*");
      }, 3500);
  } else {
...
function tryAgainGame() {
  ...
  gameon = true;
  map.random ? setMapRandom() : setMap(window.currentmap);
  TimeHandler.addEvent(function() { body.style.visibility = ""; });
  
  if (window.data) {
      window.data.score.amount = 0;
      updateDataElement(window.data.score);
      window.data.coins.amount = 0;
      updateDataElement(window.data.coins);
      window.data.lives.amount = 3;
      updateDataElement(window.data.lives);
      window.data.playerpower = 1;
  }
  if (window.player) window.player.power = 1;
}
...
function collideCastleNPC(me, collider) {
  ...
  if(window.parent && window.parent.postMessage && !map.random) {
    window.parent.postMessage({
        action: "levelComplete", 
        part: currentmap[0], 
        level: currentmap[1], power: window.player ? window.player.power : 1
    }, "*");
  }
...
function endLevelPoints(me, detector) {
  ...
  notime = nokeys = true;
  killNormal(detector);
  me.death = false; // Prevent player from shrinking when entering castle
  killNormal(me);
