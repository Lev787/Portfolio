...
function setMap(one, two) {
  ...
  if (window.parent && window.parent.postMessage) {
      window.parent.postMessage({
          action: "mapStarted",
          part: newcurrentmap[0],
          level: newcurrentmap[1],
          power: window.data ? window.data.playerpower : 1
      }, "*");
  }
...
function endCastleInsideFinal(xloc, last) {
  ...
  if(last) {
    ...
    text = stopper.text = [
      pushPreText({innerHTML: "THANK YOU " + (window.player ? window.player.title.toUpperCase() : "MARIO") + "!", style: style}, xloc + 160, 66).object,
