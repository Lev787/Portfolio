...
function expandObtainedSprite(sprite, thing, width, height, norefill) {
  var scaleKey = width + "x" + height + "x" + scale;
  if (!sprite.parsed) sprite.parsed = {};
  
  var parsed = sprite.parsed[scaleKey];
  
  if (!parsed) {
    // With the rows set, repeat them by unitsize to create the final, parsed product
    parsed = new Uint8ClampedArray(sprite.length * scale);
...
function refillThingCanvas(thing) {
  ...
  if (thing.sprite.canvasCache) {
      context.clearRect(0, 0, canvas.width, canvas.height);
      context.drawImage(thing.sprite.canvasCache, 0, 0);
  } else {
      context.clearRect(0, 0, canvas.width, canvas.height);
      ...
      var cached = getCanvas(canvas.width, canvas.height);
      cached.getContext("2d").drawImage(canvas, 0, 0);
      thing.sprite.canvasCache = cached;
  }
}
...
function refillThingCanvases(thing, parsed) {
  ...
  for(i in parsed) {
    ...
    if((part = parsed[i]) instanceof Uint8ClampedArray) {
      ...
      if (part.canvasCache) {
          context.drawImage(part.canvasCache, 0, 0);
      } else {
          imageData = context.getImageData(0, 0, width, height);
          memcpyU8(part, imageData.data);
          context.putImageData(imageData, 0, 0);
          
          var cached = getCanvas(width, height);
          cached.getContext("2d").drawImage(canvas.canvas, 0, 0);
          part.canvasCache = cached;
      }
    }
...
function refillCanvas() {
  ...
  if (typeof drawWeather !== 'undefined') drawWeather(context);
}
...
function drawThingOnCanvas(context, me) {
  ...
  var leftc = me.left,
      topc = me.top;
  if(leftc > innerWidth || me.right < 0) return;
